...
<html>
<head>
	<title>Noticias | Secundaria Oficial 0986 "Juan Rulfo"</title>
...
<div id="main-container-posicion">
	...
	<article id="main-container-content">
		<section>
			<form id="main-form" method="POST" action="../php/editarnoticia.php">
			<div id="titulo-form">
			<label>Tramites y Servicios</label>
			</div>
			...
			<table id="tabla">
				...
				<tbody id="tabla-body">
				<?php
				$consulta3="SELECT * FROM noticias WHERE PANEL='tramites' and ESTATUSBORRAR='1';";
				$resultado3 = mysqli_query($con2, $consulta3);
				while($mostrar=mysqli_fetch_array($resultado3)){
				?>
				<tr id="tablaregistro" onclick="mandar(<?php echo $mostrar['IDNOTICIA']?>)">
					<td id="idmensaje" value="<?php echo $mostrar['IDNOTICIA']?>"><?php echo $mostrar['IDNOTICIA']?></td>
					<td id="materia" value="<?php echo $mostrar['IDNOTICIA']?>"><?php echo $mostrar['ASUNTO']?></td>
					<td id="asunto" value="<?php echo $mostrar['IDNOTICIA']?>"><?php echo $mostrar['MENSAJE']?></td>
				</tr>
				<?php 
					}
				?>
